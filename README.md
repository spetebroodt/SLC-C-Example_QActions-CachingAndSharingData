...
> [!NOTE]
> This only covers sharing data across QAction runs **within your own element**. For data sharing across different element, see the [InterApp framework](https://aka.dataminer.services/InterApp).
...
This will cause some extra load due to the need to serialize and deserialize the data and the required parameter gets and sets. In most cases, this is OK as the above mentioned performance impact will be neglectable compared to other parts such as datasource access, getting and setting actual data parameters, etc.
...
This is the only option allowing to store data across element restarts by saving the parameter but important to bear in mind this can have a significant impact on storage-related costs so only save if really needed.
...
This requires the use of a static field within a static class which means the field will be shared across all elements using the same connector and running on the same SLScripting process. As a consequence, A `ConcurrentDictionary` needs to be used in order to
